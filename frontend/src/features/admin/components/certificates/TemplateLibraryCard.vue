<template>
  <ui-card class="panel">
    ...
    <form class="template-form" @submit.prevent="$emit('upload')">
      <div class="form-item"><label class="form-label">Template name</label><ui-input v-model="localTemplateForm.name" required placeholder="Summer Cup 2026" /></div>
      <div class="form-item"><label class="form-label">Image</label><input class="file-input" type="file" accept="image/*" @change="$emit('file-change', $event)" required /></div>
      <label class="check"><input v-model="localTemplateForm.is_default" type="checkbox" />Make default template</label>
      <ui-button type="submit" :disabled="isUploading" class="submit">{{ isUploading ? 'Uploading...' : 'Upload Template' }}</ui-button>
    </form>

    <ui-skeleton-loader :loading="isTemplatesLoading">
      <template #skeleton><div class="templates-grid"><ui-skeleton v-for="i in 3" :key="i" variant="rect" width="100%" /></div></template>
      ...
      <div v-else>
            ...
            <template #header>
              <div class="template-head">
                <div class="template-info">
                  ...
                  <ui-badge :variant="tpl.is_default ? 'green' : 'gray'" class="status-badge-mini">{{ tpl.is_default ? 'Default' : 'Template' }}</ui-badge>
                </div>
                <div class="mini-actions">
                  <button class="action-btn-mini" title="Edit template" @click.stop="$emit('edit', tpl)"><EditIcon class="icon-mini" /></button>
                  <button class="action-btn-mini delete" title="Delete template" @click.stop="$emit('delete', tpl.id)"><TrashIcon class="icon-mini" /></button>
                </div>
              </div>
            </template>
        ...
        <div v-if="totalTemplatePages > 1" class="pagination">
          <ui-button size="sm" variant="secondary" :disabled="templatesPage === 1" @click="$emit('prev-page')">Prev</ui-button>
          <span class="page-info">Page {{ templatesPage }} / {{ totalTemplatePages }}</span>
          <ui-button size="sm" variant="secondary" :disabled="templatesPage === totalTemplatePages" @click="$emit('next-page')">Next</ui-button>
        </div>
      </div>
    </ui-skeleton-loader>
  </ui-card>
...
const localTemplateForm = computed({ get: () => props.templateForm, set: (v) => emit('update:templateForm', v) })
</script>
...
.templates-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 0.65rem; margin-top: 0.65rem; }
.template-card { background: color-mix(in srgb, var(--background) 92%, var(--muted)); }
.template-head { display: flex; justify-content: space-between; align-items: flex-start; gap: 0.5rem; }
.template-info { display: flex; flex-direction: column; gap: 6px; flex: 1; min-width: 0; }
.template-info strong { word-break: break-word; font-size: 0.95rem; line-height: 1.2; }
.status-badge-mini { width: fit-content; font-size: 10px; padding: 2px 8px; }
.mini-actions { display: flex; align-items: center; gap: 2px; min-width: 44px; }
...
.preview { width: 100%; max-height: 220px; object-fit: contain; border-radius: 10px; border: 1px solid var(--line-soft); background: #fff; }

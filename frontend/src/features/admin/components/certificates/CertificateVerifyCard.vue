  ...
  <ui-card class="panel">
    ...
    <form class="verify-form" @submit.prevent="$emit('submit')">
      <div class="input-wrap">
        <ui-input v-model="code" placeholder="Paste code or certificate number" required class="ui-input-full" />
      </div>
    ...
    <div v-if="result" class="result" :class="result.is_valid ? 'result-valid' : 'result-invalid'">
      <div class="result-head">
        ...
        <span class="status-badge" :class="result.is_valid ? 'status-valid' : 'status-invalid'">{{ result.is_valid ? 'Valid' : 'Invalid' }}</span>
      </div>
...
.verify-form { display: flex; gap: 10px; width: 100%; }
.input-wrap { flex: 1; }
.ui-input-full { width: 100%; }
.verify-btn { min-width: 140px; }
.result { border-radius: 14px; padding: 14px; margin-top: 12px; border: 1px solid var(--line-soft); background: var(--background); }
.result-head { display: flex; justify-content: space-between; align-items: center; gap: 8px; margin-bottom: 10px; }
.result-title { margin: 0; font-size: 14px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--muted-foreground); font-weight: 700; }
...
.result-grid p { margin: 0; padding: 10px; border-radius: 10px; background: var(--muted); border: 1px solid var(--line-soft); display: flex; flex-direction: column; gap: 3px; }
.label { font-size: 12px; color: var(--color-gray-500); }
.result-message { margin: 10px 0 0; color: var(--muted-foreground); }
.result-valid { border-color: #86efac; }
.result-invalid { border-color: #fca5a5; }
</style>

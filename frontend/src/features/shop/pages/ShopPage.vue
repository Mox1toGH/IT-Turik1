  ...
  <section class="page-shell shop-page">
    <ui-card>
      <template #header>
        <div class="head">
          ...
          <div class="head-actions">
            <ui-button v-if="isAdmin" variant="secondary" @click="openCategoryCreate">Manage Categories</ui-button>
            <ui-button v-if="isAdmin" @click="openProductCreate">Create Product</ui-button>
            <ui-button v-if="isAdmin" variant="secondary" as-link to="/admin/shop-orders">Admin Orders</ui-button>
          </div>
        </div>
      </template>
      ...
      <ui-skeleton-loader :loading="isLoading">
        <template #skeleton>
          <div class="grid">
            <ui-card v-for="i in 6" :key="i"><ui-skeleton variant="rect" width="100%" height="120px" /></ui-card>
          </div>
        </template>

        <p v-if="isLoadingError" class="text-muted">Failed to load products ({{ parsedError?.message || parsedError?.code }})</p>
        <p v-else-if="!products.length" class="text-muted">No products found.</p>

        <div v-else class="grid">
          <ui-card v-for="product in products" :key="product.id" class="product-card" :class="{ unavailable: !product.is_available }">
            <template #header>
              <div class="card-head">
                <strong>{{ product.name }}</strong>
                <ui-badge :variant="product.is_available ? 'green' : 'gray'">{{ product.is_available ? 'Available' : 'Out of stock' }}</ui-badge>
              </div>
            </template>

            <img v-if="product.images[0]?.image" :src="product.images[0].image" class="thumb" alt="Product image" />
            ...
            <div class="card-actions">
              <ui-button size="sm" variant="secondary" @click="openProductDetail(product)">View</ui-button>
              <ui-button v-if="isAdmin" size="sm" @click="openProductEdit(product)">Edit</ui-button>
              <ui-button v-if="isAdmin" size="sm" variant="danger" @click="openProductDelete(product)">Delete</ui-button>
            </div>
          </ui-card>
        </div>
      </ui-skeleton-loader>
      ...
      <div v-if="totalPages > 1" class="pagination">
        <ui-button variant="secondary" :disabled="currentPage === 1" @click="currentPage -= 1">Prev</ui-button>
        <span>Page {{ currentPage }} / {{ totalPages }}</span>
        <ui-button variant="secondary" :disabled="currentPage === totalPages" @click="currentPage += 1">Next</ui-button>
      </div>
    </ui-card>
    ...
    <ui-modal v-model="isDetailOpen" maxWidth="760px">
      ...
      <div v-if="activeProduct" class="detail-body">
        <div class="image-row" v-if="activeProduct.images.length">
          <img v-for="image in activeProduct.images" :key="image.id" :src="image.image" class="detail-image" alt="Product image" />
        </div>
        ...
        <div class="purchase-box">
          ...
          <label>
            Quantity
            <ui-input v-model="purchaseQty" type="number" :min="1" :max="Math.max(1, activeProduct.stock_quantity)" />
          </label>
          <p><strong>Will deduct:</strong> {{ (purchaseQty || 1) * activeProduct.price }} points</p>
          <ui-button :disabled="isPurchasePending || !activeProduct.is_available" @click="handlePurchase">Buy</ui-button>
          <p v-if="purchaseResult" class="purchase-result">{{ purchaseResult }}</p>
        </div>
      </div>
    </ui-modal>

    <ui-modal v-model="isProductFormOpen" maxWidth="700px">
      <template #title>{{ editingProduct ? 'Edit product' : 'Create product' }}</template>
      <form class="modal-form" @submit.prevent="submitProductForm">
        <ui-input v-model="productForm.name" placeholder="Name" required />
        <ui-input v-model="productForm.description" placeholder="Description" />
        <ui-input v-model="productForm.price" type="number" :min="0" required />
        <ui-input v-model="productForm.stock_quantity" type="number" :min="0" required />
        <ui-select v-model="productForm.category_id" :options="adminCategoryOptions" />
        <ui-select v-model="productForm.product_type" :options="typeOptions" />
        <ui-select v-model="activeState" :options="activeOptions" />
        <ui-button type="submit" :disabled="isSavingProduct">{{ editingProduct ? 'Save' : 'Create' }}</ui-button>
      </form>
    </ui-modal>
      ...
      <div class="category-list">
        <ui-card v-for="category in adminCategories" :key="category.id" class="category-row">
          ...
          <div class="row-actions">
            ...
            <ui-button size="sm" variant="danger" @click="deleteCategory(category.id)">Delete</ui-button>
          </div>
        </ui-card>
        ...
        <form class="modal-form" @submit.prevent="submitCategoryForm">
          ...
          <ui-button type="submit">{{ editingCategoryId ? 'Save category' : 'Add category' }}</ui-button>
        </form>
      </div>
    </ui-modal>
  </section>
...
import type { ShopCategory, ShopProduct } from '@/api/services/shop/types'
...
const { data, isLoading, isLoadingError, error } = useShopProducts({
  ...
  pageSize,
  search,
  category: computed(() => (selectedCategory.value === 'all' ? null : Number(selectedCategory.value))),
...
const adminCategoryOptions = computed(() =>
  adminCategories.value.map((item) => ({ value: item.id, label: item.name })),
)
...
const resetProductForm = () => {
  productForm.value = {
    ...
    price: 0,
    stock_quantity: 0,
    category_id: adminCategories.value[0]?.id || 0,
...
const openProductEdit = (product: ShopProduct) => {
  ...
  activeState.value = String(product.is_active)
  isProductFormOpen.value = true
}
...
const submitProductForm = () => {
  const body = {
    ...
    is_active: activeState.value === 'true',
  }
...
<style scoped>
.shop-page { display: grid; gap: 1rem; }
...
.toolbar { display: grid; gap: 8px; grid-template-columns: repeat(4, minmax(0, 1fr)); margin-bottom: 12px; }
.grid { display: grid; gap: 10px; grid-template-columns: repeat(auto-fill, minmax(230px, 1fr)); }
.product-card { background: var(--muted); }
.product-card.unavailable { filter: grayscale(1); opacity: 0.7; }
.card-head { display: flex; justify-content: space-between; align-items: center; gap: 8px; }
.thumb { width: 100%; height: 140px; object-fit: cover; border-radius: 8px; }
.price { margin: 6px 0 0; font-weight: 700; }
.meta { margin: 2px 0; color: var(--muted-foreground); font-size: 0.85rem; }
.desc { margin: 4px 0; font-size: 0.9rem; }
...
.image-row { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 8px; }
.detail-image { width: 100%; height: 120px; object-fit: cover; border-radius: 8px; }
.purchase-box { display: grid; gap: 8px; border: 1px solid var(--border); padding: 10px; border-radius: 10px; }
.purchase-result { font-weight: 600; }
.modal-form { display: grid; gap: 8px; }
.category-list { display: grid; gap: 10px; }
.category-row { display: flex; justify-content: space-between; align-items: center; gap: 10px; }
